<!-- components/SocialConnect.vue -->
<template>
    <section class="py-16 px-4 sm:px-6 lg:px-8">
      <div class="max-w-7xl mx-auto lg:flex justify-center items-center grid lg:grid-cols-2">
        <div>
            <div class="inline-block mb-8">
          <span class="px-4 py-2 rounded-full bg-[#1d4449] text-[#c2f725] text-sm font-medium">
            Socials
          </span>
        </div>
  
        <h2 data-aos="fade-up" class="text-3xl lg:text-4xl font-bold mb-4">Connect with Us</h2>
        <p data-aos="fade-up" class="text-gray-600 mb-12 max-w-2xl">
          Join us in our community and our Social media, to stay updated about us.
        </p>
        </div>
  
        <div class="bg-gray-25 space-y-6 py-20">
   <section class="lg:divide-x divide-gray-50 lg:flex">
           <!-- Facebook -->
           <div data-aos="fade-up" class="p-6 lg:w-1/2">
            <div class="flex flex-col items-start gap-4">
              <div class="w-12 h-12 flex-shrink-0">
                <img src="@/assets/icons/facebook-icon.svg" alt="Facebook" class="w-full h-full" />
              </div>
              <div>
                <h3 class="text-xl font-semibold mb-2">Follow us on Facebook</h3>
                <p class="text-gray-600 mb-1">
                  Get CoMiles news, company information and resources
                </p>
                <a href="https://facebook.com/CoMiles_ng" class="text-[#1d4449] hover:underline">
                  @CoMiles_ng
                </a>
              </div>
            </div>
          </div>
  
          <!-- Twitter/X -->
          <div data-aos="fade-up" class="p-6 lg:w-1/2 lg:pl-10">
            <div class="flex flex-col items-start gap-4">
              <div class="w-12 h-12 flex-shrink-0">
                <img src="@/assets/icons/x-icon.svg" alt="X" class="w-full h-full" />
              </div>
              <div>
                <h3 class="text-xl font-semibold mb-2">Follow us on X</h3>
                <p class="text-gray-600 mb-1">
                  Get CoMiles news, company information and resources
                </p>
                <a href="https://x.com/CoMiles_ng" class="text-[#1d4449] hover:underline">
                  @CoMiles_ng
                </a>
              </div>
            </div>
          </div>
   </section>
  
       <section class="lg:divide-x divide-gray-50 lg:flex">
           <!-- Instagram -->
           <div data-aos="fade-up" class="p-6 lg:w-1/2">
            <div class="flex flex-col items-start gap-4">
              <div class="w-12 h-12 flex-shrink-0">
                <img src="@/assets/icons/instagram-icon.svg" alt="Instagram" class="w-full h-full" />
              </div>
              <div>
                <h3 class="text-xl font-semibold mb-2">Follow us on Instagram</h3>
                <p class="text-gray-600 mb-1">
                  Get CoMiles news, company information and resources
                </p>
                <a href="https://instagram.com/CoMiles_ng" class="text-[#1d4449] hover:underline">
                  @CoMiles_ng
                </a>
              </div>
            </div>
          </div>
  
          <!-- Call Us -->
          <div data-aos="fade-up" class="p-6 lg:w-1/2 lg:pl-10">
            <div class="flex flex-col items-start gap-4">
              <div class="w-12 h-12 flex-shrink-0">
                <img src="@/assets/icons/phone-icon.svg" alt="Phone" class="w-full h-full" />
              </div>
              <div>
                <h3 class="text-xl font-semibold mb-2">Call Us</h3>
                <p class="text-gray-600 mb-1">
                  Speak with any of our customer support.
                </p>
                <a href="tel:02018891727" class="text-[#1d4449] hover:underline">
                  02018891727
                </a>
              </div>
            </div>
          </div>
       </section>
        </div>
      </div>
    </section>
  </template>